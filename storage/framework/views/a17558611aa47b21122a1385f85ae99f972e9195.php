<div>
    ...
    <div class="p-5 lg:p-10">
        ...
        <p><?php echo nl2br(e($quiz->aciklama)); ?></p>
        <br>
        ...
        <?php $__currentLoopData = $quiz->sorular; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $soru): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex items-center pb-3">
                <h1 class="text-lg font-bold"><button
                        ...
                        class="inline rounded-md px-2 transition duration-300 ease-linear select-none focus:outline-none focus:shadow border border-yellow-600 bg-yellow-500 text-white hover:bg-yellow-600 mr-1"
                        wire:click="openEditModal(<?php echo e($soru->id); ?>)"><i class="fas fa-edit"></i>
            ...
            <?php if($soru->cevap3): ?> <p class="secenek-box <?php if($soru->dogru_cevap == 'cevap3'): ?> dogru <?php endif; ?>">c) <?php echo e($soru->cevap3); ?></p> <?php endif; ?>
            <?php if($soru->cevap4): ?> <p class="secenek-box <?php if($soru->dogru_cevap == 'cevap4'): ?> dogru <?php endif; ?>">d) <?php echo e($soru->cevap4); ?></p> <?php endif; ?>
            <br>
        ...
        <div class="text-sm flex flex-col items-center bg-gray-100 rounded-md pb-5 border border-green-200">
            ...
            <?php echo $__env->make('quiz.templates.quiz-box', ['quiz' => $quiz , 'kategori' => $kategori->isim, 'userid' => \Illuminate\Support\Facades\Auth::user()->id], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        </div>
    </div>


    <div id="modal_bg" class="modal_bg fixed left-0 bottom-0 w-full h-full bg-gray-800 transition opacity-50 invisible"
         style="transition: all .3s;">
    </div>
    <div id="modal" class="modal flex items-center justify-center fixed left-0 bottom-0 w-full h-full transition invisible"
         style="transition: all .3s;">
        <div id="modal_body" class="modal_body bg-white md:rounded-lg w-full md:w-2/3 xl:w-1/2 transition mb-30"
    ...
    <div wire:ignore id="edit_modal_bg" class="modal_bg fixed left-0 bottom-0 w-full h-full bg-gray-800 transition opacity-50 invisible"
         style="transition: all .3s;">
    </div>
    <div wire:ignore.self id="edit_modal" class="modal flex items-center justify-center fixed left-0 bottom-0 w-full h-full transition invisible"
         style="transition: all .3s;">
        <div wire:ignore.self id="edit_modal_body" class="modal_body bg-white md:rounded-lg w-full md:w-2/3 xl:w-1/2 transition mb-30"
             style="transition: all .3s; transform: scaleY(0)">
            <div class="flex flex-col p-4">
                <div class="flex items-center w-full pb-2">
                    <div id="edit_modal_baslik" class="text-gray-900 font-medium text-lg font-bold">Soru Düzenle</div>
                    <svg class="modal-kapat ml-auto fill-current text-gray-700 w-6 h-6 cursor-pointer"
                         xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
                        <path
                            d="M14.53 4.53l-1.06-1.06L9 7.94 4.53 3.47 3.47 4.53 7.94 9l-4.47 4.47 1.06 1.06L9 10.06l4.47 4.47 1.06-1.06L10.06 9z"/>
                ...
                <hr>
                <div id="edit_modal_icerik" class="modal_icerik">
                    <div class="px-5 py-3 max-h-96 md:max-h-full overflow-y-scroll md:overflow-hidden">
                        <div class="flex flex-col">
                            <form id="soru_guncelle" method="post" action="<?php echo e(route('soru_guncelle', [$quiz->uniqueid, $qid])); ?>" class="flex flex-col" enctype="multipart/form-data">
                                <?php echo csrf_field(); ?>
                            <label><b>Soruyu yaz:</b></label><br>
                            <textarea class="rounded-md mb-4" name="esoru" wire:model="editquiz.soru"></textarea><br>
                            <label><b>Soru resmi (opsiyonel):</b></label><br>
                            <input type="file" class="rounded-md w-full" name="eresim">
                            <div class="flex flex-col lg:flex-row mt-4">
                                <div class="w-full lg:w-1/2 lg:pr-5">
                                    <label><b>A şıkkı:</b></label><br>
                                    <input type="text" name="ecevap1" class="rounded-md w-full p-1" wire:model="editquiz.cevap1">
                                    <div class="mb-4"><input id="a" type="radio" wire:model="editquiz.dogru_cevap" name="dogru_cevap"
                                                             value="cevap1"><label
                                            for="a" class="text-sm">Doğru Şık</label></div>
                                    <label><b>B şıkkı:</b></label><br>
                                    <input type="text" name="ecevap2" class="rounded-md w-full p-1" wire:model="editquiz.cevap2">
                                    <div class="mb-4"><input id="b" type="radio" wire:model="editquiz.dogru_cevap" name="dogru_cevap"
                                                             value="cevap2"><label
                                            for="b" class="text-sm">Doğru Şık</label></div>
                                </div>
                                <div class="w-full lg:w-1/2">
                                    <label><b>C şıkkı:</b></label><br>
                                    <input type="text" name="ecevap3" class="rounded-md w-full p-1" wire:model="editquiz.cevap3" placeholder="Boş bırakılabilir...">
                                    <div class="mb-4"><input id="c" type="radio" wire:model="editquiz.dogru_cevap" name="dogru_cevap"
                                                             value="cevap3"><label
                                            for="c" class="text-sm">Doğru Şık</label></div>
                                    <label><b>D şıkkı:</b></label><br>
                                    <input type="text" name="ecevap4" class="rounded-md w-full p-1" wire:model="editquiz.cevap4" placeholder="Boş bırakılabilir...">
                                    <div class="mb-4">
                                        <input id="d" type="radio" name="dogru_cevap" wire:model="editquiz.dogru_cevap" value="cevap4"><label for="d" class="text-sm">Doğru
                                            Şık</label></div>
                                </div>
                            </div>
                            </form>
                ...
                <div class="ml-auto mt-5">
                    <button
                        ...
                        <button type="submit" class="btn--primary" form="soru_guncelle">
                            Kaydet
                        </button>
                </div>
            </div>
            ...
            var html = `
                          <div class="px-5 py-3 max-h-96 md:max-h-full overflow-y-scroll md:overflow-hidden">
                            <div class="">
                             ...
                             <?php echo csrf_field(); ?>
            <label><b>Soruyu yaz:</b></label><br>
             <textarea class="rounded-md mb-4" name="soru"> <?php echo e(old('soru')); ?></textarea><br>
                             <label><b>Soru resmi (opsiyonel):</b></label><br>
                              <input type="file" class="rounded-md w-full" name="resim">
                              <div class="flex flex-col lg:flex-row mt-4">
                              <div class="w-full lg:w-1/2 lg:pr-5">
                            <label><b>A şıkkı:</b></label><br>
                              <input type="text" name="cevap1" class="rounded-md w-full p-1" value="<?php echo e(old('cevap1')); ?>">
                            ...
                            <label><b>B şıkkı:</b></label><br>
                              <input type="text" name="cevap2" class="rounded-md w-full p-1" value="<?php echo e(old('cevap2')); ?>">
                            ...
                            <div class="w-full lg:w-1/2">
                            <label><b>C şıkkı:</b></label><br>
                              <input type="text" name="cevap3" class="rounded-md w-full p-1" value="<?php echo e(old('cevap3')); ?>" placeholder="Boş bırakılabilir...">
                            ...
                            <label><b>D şıkkı:</b></label><br>
                               <input type="text" name="cevap4" class="rounded-md w-full p-1" value="<?php echo e(old('cevap4')); ?>" placeholder="Boş bırakılabilir...">
